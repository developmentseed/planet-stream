...
util.inherits(ReplicationStream, Readable);

function ReplicationStream (opts) {
  if (!(this instanceof ReplicationStream)) return new ReplicationStream(opts);
  ...
  this.getReplicationSequence = opts.replicationSequence;
}

ReplicationStream.prototype._read = function () {
  var that = this;
  if (!this.started) {
    that.getReplicationSequence(that, function (err, sequence) {
...
ReplicationStream.prototype.run = function () {
  var that = this;
...
module.exports = ReplicationStream;
